...
**Ghost/reap sweeps (not in the diagram above):** `afterInit` runs `reconcileGhostPlayers` once at
boot — Redis player/room state has no TTL and outlives a process restart, but live Socket.IO
connections don't, so anything left over from before boot is deleted (`PlayersService.deletePlayer`
+ `PhysicsService.removeFromGrid`), and any room left with zero remaining players is removed.
Separately, `handleDisconnect` calls `reapIfNoRealPlayersLeft` after a real player leaves — if the
room now has bots only, it tears the room down (`BotsService.destroyRoom` +
`RoomsService.removeRoom`) rather than paying tick/network cost for bots fighting nobody.
`BotsService.tickRoom` has the same check as a third backstop (see `BotsService` below) for paths
neither of the above can see.
...
## No Postgres yet — adding player profiles

There is currently **no Postgres, no Prisma, no ORM anywhere in this repo.** (An earlier version
of this doc described a `src/prisma/` stub — that directory doesn't exist; this whole layer needs
to be built from scratch.) `package.json` has no `prisma`/`@prisma/client`/`pg` dependency, and
`docker-compose.yml` only defines `server-nest` + `redis` — no `postgres` service.

Redis (`RedisService` / player Redis hashes, see "Redis keys" below) is **live-match state only** —
it's flushed/keyed per-room and not meant to survive past a game (`RoomsService.removeRoom` deletes
it). Player profiles/accounts/stats need durable storage, which is what Postgres is for. Do not try
to repurpose the existing `player:{roomId}:{socketId}` Redis hashes for this — different lifecycle,
different shape (that hash has no persistent user id, just a per-match socket id).

To add it:

1. **`docker-compose.yml`** — add a `postgres` service (e.g. `postgres:16-alpine`, a named volume,
   `POSTGRES_USER`/`POSTGRES_PASSWORD`/`POSTGRES_DB`), and add `depends_on: postgres` to
   `server-nest` alongside the existing `redis` dependency.
2. **Dependencies** — add `prisma` (dev) + `@prisma/client` (prod) via pnpm, or a lighter client
   (`pg` + a query builder) if you want to skip an ORM. Prisma is the natural fit here since NestJS
   idioms assume a `PrismaService` injectable wrapping `PrismaClient`.
3. **New module** — create `src/database/` (or `src/prisma/`) with a `PrismaModule`/`PrismaService`
   (`onModuleInit` → `$connect()`, `onModuleDestroy` → `$disconnect()`), then a `src/profiles/`
   module (`ProfilesModule`/`ProfilesService`/`ProfilesController`) for the actual CRUD — follow the
   `PlayersService` pattern (thin data layer, no game logic) rather than putting queries in the
   Gateway.
4. **Wire it in** — import the new module into `GameModule` (REST-reachable, like `PlayersModule`)
   rather than `GatewayModule`, since profile CRUD is naturally a REST concern (`GameController` is
   the only REST controller that exists so far — see below) and doesn't need per-socket state.
5. **Where identity comes from today:** there is **no real auth yet** — `GameGateway.handleConnection`
   assigns every connection a throwaway `guest-{id}` identity (see `GameGateway` section below).
   A durable player profile needs a stable identifier that survives reconnects, which guest IDs
   don't provide — auth will likely need to land before profiles are meaningful beyond "create a row
   per session." Check whether the user wants auth stubbed alongside this or explicitly deferred.
