...
const TallGrass = memo(({
    count = 20000, // Maximum count for performance constraints
    ...
    useEffect(() => {
        ...
        function placeGrassBlade(position: THREE.Vector3, placedCount: { value: number }) {
            ...
            const grassHeight = 3.8 + Math.random() * 1.4; // Shorter (avg 2.9 vs 3.7)
    ...
    useFrame((state) => {
        ...
        const updateCount = Math.floor(count / 5);
        const startIndex = Math.floor((state.clock.elapsedTime * 10) % 5) * updateCount;
